...
void insert_trade(TRADE* trade) {
	set_env();
	char* insert_sql = "INSERT INTO trades (OFFERNUMBER, EXCHANGEACTUAL, QUANTITY, PRICE, CHARGE,COMPARISON) VALUES (:1, :2, :3, :4, :5, :6)";
	OCIHandleAlloc(envhp, (void**)&stmthp, OCI_HTYPE_STMT, 0, NULL);
	...
	OCIBindByPos(stmthp, &bnd5, errhp, 5, &trade->charge, sizeof(trade->charge), SQLT_FLT, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
	OCIBindByPos(stmthp, &bnd6, errhp, 6, &trade->comparison, sizeof(trade->comparison), SQLT_INT, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
